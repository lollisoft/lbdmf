...
CodeGenCPPStub::emit_marshaller( CORBA::IRObject_ptr obj )
{
  ...
  if( !CORBA::is_nil( t ) ) {
    ...
    if( tc->is_sequence() ) {
      ...
      CORBA::IDLType_ptr t = arr->element_type_def();
  ...
  o << "CORBA::Boolean demarshal (CORBA::DataDecoder&, StaticValueType) const;"
    << endl;
  ...
  // create
  o << "CORBA::StaticValueType " << mangled_name << "::create() const" << endl;
  o << BL_OPEN;
  ...
  case CORBA::dk_Struct:
	...
	emit_marshaller_suffix( mem[i].type_def );
	o << " ) &&" << endl;
      }
  ...
  case CORBA::dk_Union:
	...
	emit_marshaller_suffix( m.type_def );
	o << " ) )" << endl;
	o << indent << "return FALSE;" << exdent << endl;
	o << "break;" << exdent << endl;
      }
					   ...
					   CORBA::IDLType_ptr result )
{
  ...
  for( k = 0; k < p.length(); k++ ) {
    if( p[k].mode == CORBA::PARAM_OUT && is_variable_for_sii( p[k].type ) ) {
      o << ID(p[ k ].name) << " = ";
      o << "(";
      emit_idl_type_name( p[k].type_def );
      if( p[k].type->is_array() )
	o << "_slice";
      o << "*) _" << ID(p[k].name) << "._retn();" << endl;
    }
  }
				      ...
				      CORBA::IDLType_ptr result)
{
  ...
  for( i = 0; i != p.length(); i++ ) {
    if( i != 0 )
      o << ",";
    o << " ";
    emit_type_for_param( p[ i ].type_def, p[ i ].mode );
    o << " " << ID(p[ i ].name);
  }
  ...
  if (check_simple_clp_call (op) &&
	...
	emit_type_for_variable (p[i].type_def, manual);
	o << " _" << ID(p[i].name) << ";" << endl;
      }
	...
	if (p[i].type->is_valuetype()) {
	  ...
	  for (j=0; j<i; j++) {
		...
		p[j].mode == CORBA::PARAM_INOUT) {
	      ...
	      v1 = CORBA::ValueDef::_narrow (p[i].type_def);
	      v2 = CORBA::ValueDef::_narrow (p[j].type_def);
	      if (!CORBA::is_nil (v1)) {
		id1 = v1->id ();
	      }
	      if (!CORBA::is_nil (v2)) {
		...
		o << "if (((CORBA::ValueBase*)" << ID(p[i].name);
		o << ")==((CORBA::ValueBase*)" << ID(p[j].name)
		  << ")) " << BL_OPEN;
		o << "_" << ID(p[i].name) << " = ";
		emit_type_name (p[i].type_def);
		o << "::_downcast (_" << ID(p[j].name) << ");" << endl;
		o << "CORBA::add_ref (_" << ID(p[i].name) << ");" << endl;
		o << BL_CLOSE;
		have_check = true;
	      }
	    }
	  ...
	  o << "_" << ID(p[i].name) << " = ";
	  emit_type_name (p[i].type_def);
	  o << "::_downcast (" << ID(p[i].name) << "->_copy_value());" << endl;
	  if (have_check) {
	    o << BL_CLOSE;
	  }
	}
	  ...
	  o << "_" << ID(p[i].name) << " = ";
	  emit_type_name (p[i].type_def);
	  o << "::_downcast (" << ID(p[i].name)
	    << "->_to_value()->_copy_value());" << endl;
	  o << BL_CLOSE << "else " << BL_OPEN;
	  o << "_" << ID(p[i].name) << " = ";
	  emit_type_name (p[i].type_def);
	  o << "::_duplicate (" << ID(p[i].name) << ");" << endl;
	  o << BL_CLOSE;
	}
      }
    }
	...
	if (p[i].type->is_valuetype()) {
	  ...
	  for (j=0; j<i; j++) {
		...
		o << "if (((CORBA::ValueBase*)_" << ID(p[i].name);
		o << ")==((CORBA::ValueBase*)_" << ID(p[j].name)
		  << ")) " << BL_OPEN;
		o << ID(p[i].name) << " = ";
		emit_type_name (p[i].type_def);
		o << "::_downcast (" << ID(p[j].name) << ");" << endl;
		o << "CORBA::add_ref (" << ID(p[i].name) << ");" << endl;
		o << BL_CLOSE;
		have_check = true;
	      }
	    }
	  ...
	  o << ID(p[i].name) << " = ";
	  emit_type_name (p[i].type_def);
	  o << "::_downcast (_" << ID(p[i].name) << "->_copy_value());"
	    << endl;
	  if (have_check) {
	    o << BL_CLOSE;
	  }
	  ...
	  o << "if (_" << ID(p[i].name) << " && _";
	  o << ID(p[i].name) << "->_to_value()) " << BL_OPEN;
	  o << ID(p[i].name) << " = ";
	  emit_type_name (p[i].type_def);
	  o << "::_downcast (_" << ID(p[i].name)
	    << "->_to_value()->_copy_value());" << endl;
	  o << BL_CLOSE << "else " << BL_OPEN;
	  o << ID(p[i].name) << " = ";
	  emit_type_name (p[i].type_def);
	  o << "::_duplicate (_" << ID(p[i].name) << ");" << endl;
	  o << BL_CLOSE;
	}
      }
    }
    ...
    for (i=0; i<p.length(); i++) {
      if (p[i].type->is_valuetype()) {
	o << "CORBA::remove_ref (_" << ID(p[i].name) << ");" << endl;
      }
      else if (p[i].type->is_abstract_interface()) {
	o << "CORBA::release (_" << ID(p[i].name) << ");" << endl;
      }
    }
    ...
    else if (tc_result->is_valuetype()) {
      emit_type_for_variable (result, manual);
      o << " __res2;" << endl;
  ...
  o << "typedef " << absClassName << _STUB <<  " " <<
    _VC_NAMESPACE_HACK "classname;" << endl;
#endif
...
#ifdef _VC_NAMESPACE_BUG
  o << _VC_NAMESPACE_HACK  << "classname" << "::" << ID(op.name) << "(";
#else
  o << absClassName << _STUB << "::" << ID(op.name) << "(";
#endif
  for (i=0; i < p.length(); i++) {
...
void CodeGenCPPStub::emit_Interface( CORBA::InterfaceDef_ptr in )
{
  ...
  o << endl;
  o << "// Stub interface " << className << endl;
   ...
   */
  o << "void *" << absClassName;
  o << "::_narrow_helper( const char *_repoid )" << endl;
  o << BL_OPEN;
  ...
  if( bases->length() != 0 ) {
	...
	has_abstract_base = true;
      ...
      o << "void *_p;" << endl;
#ifdef _VC_NAMESPACE_BUG
      base_name = vc_emit_workaround ("", base_name);
#endif
      o << "if ((_p = ";
      o << base_name;
  ...
  /*
   * _narrow_helper2 class method
   *
   * searches downwards in the inheritance hierarchy starting at the
   * class we want to narrow to until it finds the class of the object
   * we want to narrow or until bottom is reached. Returns true
   * if _narrow succeeds
   */

  if (!_params.pseudo) {
    o << "bool " << absClassName;
    o << "::_narrow_helper2( CORBA::Object_ptr _obj )" << endl;
    o << BL_OPEN;
    o << "if( strcmp( _obj->_repoid(), \"" << id << "\" ) == 0) {" << endl;
    o << indent << "return true;" << exdent << endl;
    o << "}" << endl;
    o << "for( vector<CORBA::Narrow_proto>::size_type _i = 0;" << endl;
    o << "     _narrow_helpers && _i < _narrow_helpers->size(); _i++ ) ";
    o << BL_OPEN;
    o << "bool _res = (*(*_narrow_helpers)[ _i ])( _obj );" << endl;
    o << "if( _res )" << endl;
    o << indent << "return true;" << exdent << endl;
    o << BL_CLOSE;
    o << "return false;" << endl;
   ...
   * if that fails generate a stub for it using the _narrow_helper2
   * method. if that fails too call _obj->_is_a (_repoid) and create a
   * stub if _is_a() returns true. otherwise return _nil().
   */
  o << absClassName << "_ptr " << absClassName;
  o << "::_narrow( CORBA::Object_ptr _obj )" << endl;
  o << BL_OPEN;
  if (!_params.pseudo) {
    if (!in->is_abstract()) {
  ...
  o << indent
    ...
    << exdent << endl;
  if (!_params.pseudo) {
    o << "if( _narrow_helper2( _obj ) ||" << endl;
    o << "   ( ";
    if (!_params.query_server_for_narrow)
      o << "!strlen (_obj->_repoid()) && ";
    o << "_obj->_is_a_remote( \"" << id << "\" ) ) ) " << BL_OPEN;
    o << "_o = new " << absClassName;
    o << _STUB << ";" << endl;
    o << "_o->";
    // Win32 hack
    o << "MICO_SCOPE(CORBA,Object::operator=)( *_obj );" << endl;
    o << "return _o;" << endl;
    o << BL_CLOSE;
  }
  o << BL_CLOSE;
  ...
  if (_params.poa) {
    ...
    for (CORBA::ULong p1=0; p1<bases->length(); p1++) {
      ...
      o << "if ((p = POA_" << base_name <<
        "::_narrow_helper (repoid)) != NULL) " << endl;
      o << BL_OPEN;
      o << "return p;" << endl;
      o << BL_CLOSE;
    }
	...
	<<  " (PortableServer::POA_ptr poa, CORBA::Object_ptr obj)" << endl;
      o << "  : MICO_SCOPE(PortableServer,StubBase) (poa), MICO_SCOPE(CORBA,Object) (*obj)" << endl;
      o << "{" << endl;
      o << "}" << endl << endl;
      o << absClassName << _STUB << "_clp::~"
	<<    className << _STUB << "_clp ()" << endl;
  ...
  for( CORBA::ULong j = 0; j < dsv->length(); j++ ) {
    ...
    // op.exceptions does not need to be set for attributes

    string dispatch_name = string("_get_") + (const char *)attr->name;
    
    CORBA::IDLType_var idl_type = lookup_attribute_by_id( attr->id );
    ...
    if( attr->mode == CORBA::ATTR_NORMAL ) {
      ...
      attr_param.type_def = lookup_attribute_by_id( attr->id );
  ...
  // set all _narrow_helper2 methods in their respective
  // base classes
  if( bases->length() != 0 ) {
    o << "struct _global_init_";
    emit_mangled_name( (char *) absClassName.c_str() );
    o << " " << BL_OPEN;
    o << "_global_init_";
    emit_mangled_name( (char *) absClassName.c_str() );
    o << "()" << endl;
    ...
    for( CORBA::ULong i = 0; i < bases->length(); i++ ) {
      o << "if( ";
      CORBA::String_var abs = bases[ i ]->absolute_name();
      o << abs << "::_narrow_helpers == NULL )" << indent << endl;
      o << abs << "::_narrow_helpers = new vector<CORBA::Narrow_proto>;"
	<< exdent << endl;
      o << abs << "::_narrow_helpers->push_back( ";
      o << absClassName << "::_narrow_helper2 );" << endl;
    }
    o << BL_CLOSE;
    o << exdent << "} __global_init_";
    emit_mangled_name( (char *) absClassName.c_str() );
    o << ";" << endl << endl;
  }
    
  // Make instance for static member variable _narrow_helpers
  emit_var_def ("vector<CORBA::Narrow_proto> *", in, "_narrow_helpers", "");
...
/*
 * splitted emit_Interface() to work around gcc 2.7.2 internal
 * compiler error. kay.
 */
void CodeGenCPPStub::emit_Interface2( CORBA::InterfaceDef_ptr in )
{
...
void CodeGenCPPStub::emit_Union( CORBA::UnionDef_ptr u )
{
  ...
  if (contains_a_valuetype (u)) {
    ...
    for (CORBA::ULong i0=0; i0 < members->length(); i0++) {
      string prefix = ID(members[i0].name);
      prefix += "()";
    ...
    for (CORBA::ULong i1=0; i1 < members->length(); i1++) {
      string prefix = ID(members[i1].name);
      prefix += "()";
...
void CodeGenCPPStub::emit_Constant( CORBA::ConstantDef_ptr c )
{
  ...
  if( def_in->def_kind() == CORBA::dk_Repository )
    // This ConstantDef was defined on global scope, we
    // need to do nothing.
    return;
...
void CodeGenCPPStub::emit_Exception( CORBA::ExceptionDef_ptr e )
{
  ...
  o << "void " << abs << name
    << "::_encode( CORBA::DataEncoder &_en ) const" << endl;
  o << BL_OPEN;
  if( _params.pseudo ) {
    o << "assert(0);" << endl;
  } else {
  ...
  o << "void " << abs << name
    << "::_encode_any( CORBA::Any &_a ) const" << endl;
  o << BL_OPEN;
  if (_params.any) {
    o << "_a <<= *this;" << endl;
  } else {
  ...
  o << abs << name << " *" << abs << name;
  o << "::EXCEPTION_DOWNCAST( CORBA::Exception *_ex )" << endl;
  o << BL_OPEN;
  ...
  o << "::EXCEPTION_DOWNCAST( const CORBA::Exception *_ex )" << endl;
  o << BL_OPEN;
  o << "if( _ex && !strcmp( _ex->_repoid(), \"" << repoid << "\" ) )" << endl;
  o << indent << "return (" << name << " *) _ex;" << exdent << endl;
  o << "return NULL;" << endl;
			      ...
			      const string &init)
{
	...
	o << "namespace " << ID(modules[modules.size()-1-i].c_str()) << " { ";
    }
    o << type << " " << ID(name.c_str()) << init << ";";
    ...
    o << type << " ";
    for (mico_vec_size_type k = 0; k < modules.size(); ++k) {
	o << ID(modules[modules.size()-1-k].c_str()) << "::";
    }
    o << ID(name.c_str()) << init << ";" << endl;
...
CodeGenCPPStub::emit_Value (CORBA::ValueDef_ptr val)
{
  ...
  for (CORBA::ULong i01=0; i01<supported->length(); i01++) {
    ...
    bases_name = bases_name.substr( 2 );
#ifdef _VC_NAMESPACE_BUG
    bases_name = vc_emit_workaround ("", bases_name);            
#endif
     o << "if ((p = " << bases_name <<
      "::_narrow_helper (repoid)) != NULL) " << BL_OPEN;
    o << "return p;" << endl;
  ...
  o << "CORBA::Repository_var ifr = CORBA::Repository::_narrow (irobj);"
    << endl;
  o << "assert (!CORBA::is_nil (ifr));" << endl << endl;
  o << "CORBA::Contained_var cv = ifr->lookup_id (\"" << id << "\");"
    << endl;
  o << "CORBA::ValueDef_ptr val = CORBA::ValueDef::_narrow (cv);"
    << endl;
  o << "return val;" << endl;
  o << BL_CLOSE << endl;
  ...
  if (!val->is_abstract()) {
    ...
    if (members->length() > 0) {
      ...
      for (CORBA::ULong j0=0; j0<members->length(); j0++) {
	CORBA::ValueMemberDef_var vmd =
	  CORBA::ValueMemberDef::_narrow (members[j0]);
	CORBA::IDLType_var idt = vmd->type_def ();
	CORBA::String_var name = vmd->name ();
	CORBA::TypeCode_var tc = idt->type ();
	/*
	...
	if (j0+1 != members->length()) {
	  o << ", ";
	}
      }
      o << ")" << endl;
      o << BL_OPEN;
      for (CORBA::ULong j1=0; j1<members->length(); j1++) {
	CORBA::ValueMemberDef_var vmd =
	  CORBA::ValueMemberDef::_narrow (members[j1]);
	CORBA::IDLType_var idt = vmd->type_def ();
	CORBA::String_var name = vmd->name ();
	string p_name = "_";
	p_name += name.in();
	emit_union_member_assigner (idt, name, "_m.", p_name.c_str());
      }
      o << BL_CLOSE << endl;
    }
...
CodeGenCPPStub::emit_ValueBox (CORBA::ValueBoxDef_ptr vb)
{
  ...
  case CORBA::tk_sequence:
	...
	<< className << "& _p)" << endl;
      ...
      o << absClassName << "::" << className << " (CORBA::ULong max)" << endl;
      o << BL_OPEN;
	...
	<<   "_elem_type * data, CORBA::Boolean rel)" << endl;
      ...
      o << "const " << absClassName << "::_elem_type &" << endl;
      ...
      o << "void" << endl;
      o << absClassName << "::replace (CORBA::ULong max, CORBA::ULong length, "
	<<                            "_elem_type * data, CORBA::Boolean rel)"
	<< endl;
      o << BL_OPEN;
  ...
  if (boxed_value->def_kind() == CORBA::dk_Sequence) {
    o << absClassName << "::_value_type *&";
  }
  else if (orig->def_kind() == CORBA::dk_Array) {
    emit_type_name (boxed_value);
   ...
   */
  o << "CORBA::Boolean" << endl;
  o << absClassName << "::_demarshal_members (CORBA::DataDecoder &ec)" << endl;
  o << BL_OPEN;
  o << "return ";
  emit_marshaller_ref (orig.in());
