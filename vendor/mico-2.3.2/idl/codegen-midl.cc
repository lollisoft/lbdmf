...
CodeGenMIDL::CodeGenMIDL( DB &db, bool emit_repoids )
...
void CodeGenMIDL::emitForwardDcl( CORBA::Container_ptr c )
{
  if (c->def_kind() == CORBA::dk_Interface) {
    ...
    if (in->is_abstract()) {
      //o << "abstract ";
      cerr << "no translation for abstract! " << name << endl;
    }
    ...
    cerr << "no translation for valuetype! " << name << endl;
  }
  else {
    assert (0);
  }
...
void CodeGenMIDL::emitIDLInterface( CORBA::InterfaceDef_ptr in )
{
  ...
  if (in->is_abstract()) {
    //o << "abstract ";
    cerr << "warning: abstract ignored : " << className << endl;
  }
