...
SequenceIndTmpl<T_elem,T,n>::SequenceIndTmpl (MICO_ULong max,
					      MICO_ULong length,
					      T* value,
					      MICO_Boolean rel)
{
...
template<class T_elem, class T, MICO_ULong n>
void
SequenceIndTmpl<T_elem,T,n>::replace (MICO_ULong max,
				      MICO_ULong length,
				      T* value,
				      MICO_Boolean rel)
{
  ...
  vec.reserve (max);
  for (MICO_ULong i1 = 0; i1 < length; ++i1) {
    T_elem* new_elem = new T_elem[ n ];
    T_elem* dest = new_elem;
...
template<class T_elem, class T, MICO_ULong n, int max>
void
BoundedSequenceIndTmpl<T_elem,T,n,max>::replace (MICO_ULong length, T *value,
						 MICO_Boolean rel)
{
  assert (length <= max);
  for( mico_vec_size_type i0 = 0; i0 < vec.size(); i0++ )
