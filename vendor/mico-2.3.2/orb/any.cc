...
/*
 * whether or not to allow structural subtyping ala ODP-RM during
 * extraction from any...
 */
#define ALLOW_SUBTYPING
...
CORBA::Any::Any ()
{
    ...
    ec = new MICO::SimpleEncoder ();
    dc = new MICO::SimpleDecoder (ec->buffer(), FALSE);
...
CORBA::Any::Any (TypeCode_ptr tc, Buffer *b, Boolean release, int /*dummy*/)
{
    ...
    checker = new TypeCodeChecker ();
    ec = new MICO::SimpleEncoder (b, FALSE);
    dc = new MICO::SimpleDecoder (b, release);
    ec->valuestate (&value_estate, FALSE);
    dc->valuestate (&value_dstate, FALSE);
    extracted_value = 0;
}
...
CORBA::Any::copy_any (Any &a, MapLL &vidmap, Boolean recurse)
{
	...
	Object_var obj;
	String_var name;
	return ((a >>= to_object (obj, &name.inout())) &&
    ...
    case tk_except: {
	String_var repoid;
	if (!a.except_get_begin (repoid.out()) || !except_put_begin (repoid))
	    return FALSE;
...
CORBA::Any::compare_any (Any &a, MapLL &vidmap)
{
	...
	if (!a.except_get_begin (repoid1.out()) ||
	    !except_get_begin (repoid2.out()) || strcmp (repoid1, repoid2))
	    return FALSE;
...
CORBA::Any::marshal (DataEncoder &e)
{
    ...
    Any a (tc(), new MICO::SimpleDecoder,
...
CORBA::Boolean
CORBA::Any::operator>>= (Any &a) const
{
    Any &me = (Any &)*this;

    me.prepare_read ();
    if (!me.checker->basic (_tc_any) || !me.dc->get_any (a)) {
	me.rewind ();
	return FALSE;
    }
    return TRUE;
...
CORBA::Boolean
CORBA::Any::operator>>= (wchar_t * &s) const
{
    wchar_t **p;
...
CORBA::Any::struct_get_end () const
{
    ...
    if (!me.checker->inside (tk_struct)) {
	me.rewind ();
	return FALSE;
    }
    Any a;
    while (!me.checker->level_finished ()) {
	CORBA::Boolean r = any_get (a);
	assert (r);
    }
#endif
...
CORBA::Boolean
CORBA::Any::except_get_begin (char *&repoid) const
{
    Any &me = (Any &)*this;
...
CORBA::Any::except_get_end () const
{
    ...
    if (!me.checker->inside (tk_except)) {
	me.rewind ();
	return FALSE;
    }
    Any a;
