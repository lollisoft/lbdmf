...
-->
<xsl:import href="XMISysImport.FillSchemaTables.Sqlite.xsl"/>
<!--
...
<xsl:template name="XMISysImport.FillSchemaTables">
...
<xsl:call-template name="XMISysImport.FillSchemaTables.Sqlite">
...
<xsl:template name="XMISysImport.FillSchemaAssociations">
	<xsl:param name="overwriteDatabase"/><!-- When set to yes, DROP rules are created -->
    <xsl:param name="ApplicationName"/>
...
<xsl:choose>
<xsl:when test="$TargetDBType='Sqlite'">
<xsl:call-template name="XMISysImport.FillSchemaAssociations.Sqlite">
