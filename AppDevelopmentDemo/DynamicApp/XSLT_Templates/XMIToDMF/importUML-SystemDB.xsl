...
-->
		...
		-- Class <xsl:value-of select="@name"/> of type REPORT found.
<!--		
<xsl:call-template name="log_message">
...
WHERE "src_actionid" IN (SELECT "id" FROM "action_steps" WHERE "actionid" = (select "id" from "actions" where "target" = '<xsl:value-of select="$ID"/>'))
   OR "dst_actionid" IN (SELECT "id" FROM "action_steps" WHERE "actionid" = (select "id" from "actions" where "target" = '<xsl:value-of select="$ID"/>'))
DELETE FROM "action_step_parameter" WHERE "action_step_id" IN (select "id" from "action_steps" where "bezeichnung" = '<xsl:value-of select="../../@xmi:id"/>');
DELETE FROM "action_steps" WHERE "actionid" = (select "id" from "actions" where "target" = '<xsl:value-of select="$ID"/>');
DELETE FROM "actions" where "target" = '<xsl:value-of select="$ID"/>';
	<xsl:choose>
		<xsl:when test="$nodetype='uml:InitialNode'">
		
INSERT INTO "action_steps" ("actionid", "bezeichnung", "a_order_nr", "type", "what") VALUES ((select "id" from "actions" where "target" = '<xsl:value-of select="$ID"/>'), '<xsl:value-of select="./@xmi:id"/>', '<xsl:value-of select="$step"/>', (select "id" from "action_types" where "bezeichnung" = 'InitialNode'), '');
		</xsl:when>
		<xsl:when test="$nodetype='uml:SendSignalAction'">
		<xsl:variable name="comment" select="./ownedComment/@body"/>
INSERT INTO "action_steps" ("actionid", "bezeichnung", "a_order_nr", "type", "what") VALUES ((select "id" from "actions" where "target" = '<xsl:value-of select="$ID"/>'), '<xsl:value-of select="./@xmi:id"/>', '<xsl:value-of select="$step"/>', (select "id" from "action_types" where "bezeichnung" = 'SendSignalAction'), '<xsl:value-of select="$comment"/>');
<xsl:for-each select="./xmi:Extension/taggedValue">
INSERT INTO "action_step_parameter" ("name", "value", "interface", "description", "action_step_id") VALUES ('<xsl:value-of select="./@tag"/>', '<xsl:value-of select="./@value"/>', 'lb_I_String', 'A description ...', (select "id" from "action_steps" where "bezeichnung" = '<xsl:value-of select="../../@xmi:id"/>'));
</xsl:for-each>
		...
		<xsl:when test="$nodetype='uml:DecisionNode'">
INSERT INTO "action_steps" ("actionid", "bezeichnung", "a_order_nr", "type", "what") VALUES ((select "id" from "actions" where "target" = '<xsl:value-of select="$ID"/>'), '<xsl:value-of select="./@xmi:id"/>', '<xsl:value-of select="$step"/>', (select "id" from "action_types" where "bezeichnung" = 'DecisionNode'), '');
		</xsl:when>
		<xsl:when test="$nodetype='uml:OpaqueAction'">
		<xsl:variable name="comment" select="./ownedComment/@body"/>
INSERT INTO "action_steps" ("actionid", "bezeichnung", "a_order_nr", "type", "what") VALUES ((select "id" from "actions" where "target" = '<xsl:value-of select="$ID"/>'), '<xsl:value-of select="./@xmi:id"/>', '<xsl:value-of select="$step"/>', (select "id" from "action_types" where "bezeichnung" = 'OpaqueAction'), '<xsl:value-of select="$comment"/>');
		</xsl:when>
		<xsl:when test="$nodetype='uml:FinalNode'">
INSERT INTO "action_steps" ("actionid", "bezeichnung", "a_order_nr", "type", "what") VALUES ((select "id" from "actions" where "target" = '<xsl:value-of select="$ID"/>'), '<xsl:value-of select="./@xmi:id"/>', '<xsl:value-of select="$step"/>', (select "id" from "action_types" where "bezeichnung" = 'FinalNode'), '');
		</xsl:when>
		<xsl:when test="$nodetype='uml:ActivityParameterNode'">
INSERT INTO "action_parameters" ("name", "value", "interface", "description", "actionid") VALUES ('<xsl:value-of select="./@name"/>', '', 'lb_I_String', 'A description ...', (select "id" from "actions" where "target" = '<xsl:value-of select="$ID"/>'));
		</xsl:when>
		<xsl:otherwise>
-- Nodetype <xsl:value-of select="$nodetype"/> not known.
		</xsl:otherwise>
