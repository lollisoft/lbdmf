	...
	while (trans->hasMoreActionStepTransitions()) {
		...
		if (trans->getActionStepTransitionSrcActionID() == id) {
			...
			*expression = trans->getActionStepTransitionDecision();
			...
			_LOG << "Evaluate expression '" << expression->charrep() << "' for transition = " << trans->getActionStepTransitionID() <<
			...
			if (expression->strpos("==") != -1) {
				...
				if (expression->strpos("!=") != -1) {
					...
					if (expression->strpos("=") != -1) {
						...
						left = expression->left(expression->strpos("=")-1);
						right = expression->right(expression->strpos("=")+1);
						...
						_LOG << "Have build left = '" << left->charrep() << "' and right = '" << right->charrep() << "' from expression = '" << expression->charrep() << "'" LOG_
...
/*...e*/
		...
		*msg = "Execute delegated action '";
		*msg += action->getClassName();
		*msg += "'";
		...
		if (query->query(q) == ERR_NONE) {
			...
			meta->setStatusText("Info", "Error: Query for action handlers didn't found any handlers.");
		}
	}
	return nextStep;
}
	...
	if (appActionSteps != NULL) {
		...
		_LOG << "Get action steps from id = " << myActionID << ". Query is " << q LOG_
			...
			if (query->query(q) == ERR_NONE) {
				...
				UAP_REQUEST(getModuleInstance(), lb_I_String, errmsg)
				*errmsg = "Fehler: Abfrage '";
				*errmsg += buf;
				*errmsg += "' hat keine Daten geliefert.!";
				meta->setStatusText("Info", errmsg->charrep());
			}
	}
}
/*...e*/
