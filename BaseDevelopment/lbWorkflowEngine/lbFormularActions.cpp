...
/*...slb_I_Action\42\ lbFormularActions\58\\58\getAction\40\char\42\ id\41\:0:*/
lb_I_Action* lbFormularActions::getAction(long id) {
	lbErrCodes err = ERR_NONE;
...
long lbFormularActions::getActionTargetIDLong(const char* reversed_event) {
	lbErrCodes err = ERR_NONE;
	if (eventmapping != NULL) {
...
/*...schar\42\ lbFormularActions\58\\58\getActionTargetID\40\char\42\ what\41\:0:*/
char* lbFormularActions::getActionTargetID(const char* reversed_event) {
	lbErrCodes err = ERR_NONE;
	...
	if (((err = query->first()) == ERR_NONE) || (err == WARN_DB_NODATA)) {
		...
		_CL_VERBOSE << "lbFormularActions::getActionTargetID('" << What->charrep() << 
		"') returns '" << source->charrep() << "'" LOG_
		
		return strdup(source->charrep());
	}
	
	_CL_LOG << "lbFormularActions::getActionTargetID('" << What->charrep() << "') failed!" LOG_
		
	return strdup("");
}
/*...e*/
/*...schar\42\ lbFormularActions\58\\58\getActionSourceDataField\40\char\42\ what\41\:0:*/
char* lbFormularActions::getActionSourceDataField(const char* reversed_event) {
	lbErrCodes err = ERR_NONE;
	UAP_REQUEST(getModuleInstance(), lb_I_MetaApplication, meta)
...
/*...e*/
/*...schar\42\ lbFormularActions\58\\58\getActionID\40\char\42\ what\41\:0:*/
long lbFormularActions::getActionID(const char* what) {
	lbErrCodes err = ERR_NONE;
