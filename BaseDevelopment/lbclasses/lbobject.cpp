...
IMPLEMENT_FUNCTOR(instanceOfULong, lbULong)
...
#ifdef _MSC_VER
// 1. Add this helper right above str_replace so VC6 knows the exact return type
const char* win_strcasestr(const char* haystack, const char* needle) {
...
#ifdef _MSC_VER
        if (nocase) {
                if ( (p=win_strcasestr(str, sub_str1)) == NULL )
...
#ifdef _MSC_VER
        if (nocase) {
                for ( p+=len; (p=win_strcasestr(p, sub_str1)) != NULL; p+=len )
                        ++count;
        } else {
...
void LB_STDCALL lbString::setString(char const * p) {
        if (stringdata != NULL) free(stringdata);
...
void LB_STDCALL lbString::appendString(char const * p) {
        *this += p;
}
...
void lbULong::setData(unsigned long p) {
        longdata = p;
...
unsigned long lbULong::getData() const {
        return longdata;
}

BEGIN_IMPLEMENT_LB_UNKNOWN(lbULong)
        ADD_INTERFACE(lb_I_ULong)
        ADD_INTERFACE(lb_I_KeyBase)
END_IMPLEMENT_LB_UNKNOWN()

lbErrCodes LB_STDCALL lbULong::setData(lb_I_Unknown* uk) {
        lbErrCodes err = ERR_NONE;
        UAP(lb_I_ULong, l)

        if (uk != NULL) {
                QI(uk, lb_I_ULong, l)
                unsigned long _l = l->getData();
                setData(_l);
        }
...
/*...slbKeyUL:0:*/
char const* LB_STDCALL lbULong::getKeyType() const {
    return "UL";
}

int LB_STDCALL lbULong::equals(const lb_I_KeyBase* _key) const {
    return longdata == ((lbULong*) _key)->longdata;
}

int LB_STDCALL lbULong::greater(const lb_I_KeyBase* _key) const {
    return longdata > ((lbULong*) _key)->longdata;
}

int LB_STDCALL lbULong::lessthan(const lb_I_KeyBase* _key) const {
    return longdata < ((lbULong*) _key)->longdata;
}

char* LB_STDCALL lbULong::charrep() const {
        return key;
}
/*...e*/
/*...e*/
